...
from app.services.skill_services import create_skill, get_skill_by_id, get_skills_for_user, update_skill, remove_skill
...
def delete_skill(skill_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    remove_skill(db, skill_id, current_user)
